...
def main(yolo):
    ...
    for frame in all_frames:
        ...
        for track in tracker.tracks:
            ...
            if bbox[0] >= 0 and bbox[1] >= 0 and bbox[3] < h and bbox[2] < w and abs(bbox[3]-bbox[1]) > 20 and abs(bbox[2]-bbox[0]) > 20:
            ...
            if len(exist_ids) == 0:
                ...
                for nid in new_ids:
                    ...
                    if dis[0][1] < threshold:
                        combined_id = dis[0][0]
                        print(f"Combined id={combined_id} new id={nid}")
                        feat_combined=face_network._features(images_by_id[combined_id])
                        feat_nid=face_network._features(images_by_id[nid])
                        flag=0
                        if feat_combined != None and feat_nid != None:
                            fc_dist=np.mean(face_network.compute_distance(feat_combined,feat_nid))
                            print("face distance when body feature are same:",fc_dist)
                            if fc_dist>face_threshold:
                                final_fuse_id[nid]=[nid]
                                flag=1
                        if flag == 0:
    ...
    for idx in final_fuse_id.keys():
        print(idx)
        feats[idx] = face_network._features(images_by_id[idx])
        if feats[idx] == None:
            fuse_face[idx]=final_fuse_id[idx]
    fuse_keys=[ i for i in final_fuse_id.keys() ]
    
    
    print("fuse_key:",fuse_keys)
    for idx in fuse_keys:
        # print("idx:",idx)
        if feats[idx] != None and final_fuse_id[idx] != []:
            fuse_face[idx]=final_fuse_id[idx]
        
            for cmp_id in fuse_keys:
                # print("cmp_idx:",cmp_id)
                if idx != cmp_id and feats[cmp_id] != None and final_fuse_id[cmp_id] != []:
                    print(f"Id:{idx} cmp_id:{cmp_id}")
                    tmp = np.mean(face_network.compute_distance(feats[idx], feats[cmp_id]))
                    print("temporary dist:",tmp)
                    if tmp < face_threshold:
                        # final_fuse_id[id].append(cmp_id)
                        print(" Dist less than threshold:",tmp)
                        print(f"ids combined by face detection: ID: {idx} Cmp_id{cmp_id}")
                        fuse_face[idx].extend(final_fuse_id[cmp_id])
                        images_by_id[idx] += images_by_id[cmp_id]
                        final_fuse_id[cmp_id]=[]
            final_fuse_id[idx]=[]

    print('******After FaceNet Final ids and their sub-ids:*****', fuse_face)
    final_fuse_id=fuse_face
