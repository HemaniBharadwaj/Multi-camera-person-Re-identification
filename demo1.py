...
def main(yolo):
    ...
    for i in images_by_id:
        ...
        clustering = DBSCAN(metric='euclidean', eps=threshold, min_samples=2)
                ...
                if len(exist_ids) == 0:
                    ...
                    for nid in new_ids:
                        ...
                        for oid in (exist_ids - set(unpickable)) & set(final_fuse_id.keys()):
                            oid_features = feats[oid]
                            nid_features = feats[nid]
                            _, indices = knn.kneighbors(nid_features)
                            if indices[0][0] in final_fuse_id[oid]:
                                continue
                            if np.linalg.norm(oid_features[0] - all_features[indices[0][0]]) <= threshold:
                                final_fuse_id[oid].append(indices[0][0])
                                break
                        else:
